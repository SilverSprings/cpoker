  ...
  constructor() { }
	...
	bestHand (cards) {
		...
		if (hand = isStraightFlush(cards)) {
			type = 'Straight Flush';
			val = 8;
		} else if (hand = isFourOfAKind(cards)) {
			type = 'Four of a Kind';
			val = 7;
			...
			type = 'Straight';
			val = 4;
		} else if (hand = isThreeOfKind(cards)) {
			type = 'Three of a Kind';
			val = 3;
		} else if (hand = isTwoPairs(cards)) {
			type = 'Two Pairs';
			val = 2;
		} else if (hand = isOnePair(cards)) {
			type = 'One Pair';
			val = 1;
		} else {
		...
		if (hand.length == 5) subval = subValCalc(val,[hand[0].v,hand[1].v,hand[2].v,hand[3].v,hand[4].v]);

		if (hand.length == 3) subval = subValCalc(val,[hand[0].v,hand[1].v,hand[2].v,0,0]);
...
function isStraightFlush (cards) {
	...
	for (var j=0; j < suits.length; j++) {
		var straightFlush = isStraight(suits[j]);
...
function isFlush (cards) {
	...
	let suitInteger = suits.map( suits => subValCalc(5, suits));
...
function isFourOfAKind (cards) {
	...
	var matchFour = isXOfKind(cards, 4);
	...
	if (remainingCards[0].v == 1) highCard = remainingCards[0];  
...
function isFullHouse (cards) {
	...
	var matchThree = isXOfKind(cards, 3);
	...
	var highPair = isXOfKind(remainingCards, 2);
...
function isThreeOfKind (cards) {
	...
	if (remainingCards.length != 0) {
		...
		if (remainingCards[0].v == 1) { 
			twoHighCards.push(remainingCards[0]); 
...
function isTwoPairs (cards) {
	...
	var highPair1 = isXOfKind(cards, 2);
	...
	var highPair2 = isXOfKind(remainingCards1, 2);		
	...
	if (remainingCards2[0].v == 1) highCard = remainingCards2[0]; 
...
function isOnePair (cards) {
	...
	//if no pair is found, highPair = false
	var highPair = isXOfKind(cards, 2);
	...
	if (remainingCards.length != 1) {
		...
		if (remainingCards[0].v == 1) { 
			threeHighCards.push(remainingCards[0]); 
...
function subValCalc (val, subValue)  {
